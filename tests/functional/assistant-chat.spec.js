...
const { AssistantChat } = require('../../pages/AssistantChat');
const { SITE_URL } = require('../../utils/constants');

test.describe('Functional: Carlo Assistant widget', () => {
  ...
  test('Carlo Assistant toggle is visible', async ({ page }) => {
    const ai = new AssistantChat(page);

    await expect(ai.toggleButton).toBeVisible();
  });

  test('Carlo Assistant opens successfully', async ({ page }) => {
    const ai = new AssistantChat(page);
  ...
  test('Voice input is available', async ({ page }) => {
    ...
    expect(
      voiceInput,
      'Carlo Assistant voice input button should exist'
    ).not.toBeNull();
