...
| TC-064 | GitHub profile link is reachable | Homepage loaded | Browser-navigate to GitHub URL | Response status < 400 | Medium | Automated |
| TC-064b | LinkedIn profile link is well-formed | Homepage loaded | Check href matches expected LinkedIn URL pattern | href exactly matches expected profile URL | Medium | Automated (structural only — see Known Limitations) |
...
- **LinkedIn live reachability** — confirmed via two independent technical approaches (a raw HTTP client with realistic browser headers, and a real Playwright browser page navigation) that linkedin.com blocks automated/CI-origin requests regardless of client fidelity — almost certainly IP-based blocking of datacenter/CI ranges, not something fixable client-side. TC-064b checks the href matches the exact expected profile URL instead; live reachability of this specific link should be spot-checked manually if ever in doubt.
